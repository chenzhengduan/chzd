...
熟悉 Vue、React、TypeScript、Vite/Webpack、Element Plus、Vant、Naive UI 等前端技术栈，具备 Flutter、小程序、H5、PC 管理后台及 Java/Spring Boot WebFlux 后端开发经验。能够在前端开发、全栈开发、前端组长等角色中承担业务交付、架构优化、任务拆解、组件规范和技术指导工作。
...
- 具备 AI 应用工程落地经验，参与 AgentScope 大模型能力与业务系统集成，处理流式生成、异步任务、多模态输入、结果复核和失败恢复等完整交互链路。
...
- 熟悉基于 Fetch + ReadableStream 的 SSE 流式交互，能够处理生成进度、超时检测、主动中断、重复请求控制、异常提示和结果回填，并结合任务查询与轮询机制恢复异步任务状态。

### 后端与全栈

- 熟悉 Java、Spring Boot、Spring WebFlux、Spring Cloud，具备响应式接口、异步任务、定时任务、数据迁移、文件服务和后台管理系统开发经验。
...
- 负责或参与 AI 智能出卷与录题平台、AI 家校圈、师生信小程序、新版排课、Flutter App、云学辅上传稳定性等项目，覆盖 AI 流式生成、复杂试题渲染、多模态录题、项目搭建、性能优化、跨端适配和稳定性治理。
...
### AI 智能出卷与测评平台｜核心前端 / 部分 Java 后端研发

- **角色：**负责核心前端及部分 Java 后端研发，参与需求拆解、方案设计、接口联调与功能交付。
- **业务：**覆盖配置式/对话式 AI 出卷、试卷管理、在线答题与批改、单题及整卷变式，以及图片/PDF/Word 智能录题。
- 基于 Vue 3、TypeScript、Pinia 和 Element Plus 搭建核心业务界面，处理学科年级联动、题型分配、试卷生命周期、答题卡联动、答案解析和多主题适配等复杂状态。
- **技术难点：**封装基于 Fetch + ReadableStream 的 SSE 流式交互，支持 POST 流式请求、实时生成进度、超时检测、主动中断、重复请求控制、异常提示和结果回填。
- **复杂渲染：**完成数学公式、题组、多空填空、子题、答案解析和题内图等试题结构的统一渲染，保障生成、预览、在线答题、批改和打印场景的一致性。
- **工程链路：**打通 OSS 直传、异步识别、任务查询与轮询恢复、跨页题内图裁剪、识别结果复核、答案重生成和批量入库。
- **后端协作：**参与 Spring Boot WebFlux 接口和数据结构建设，实现试卷/答卷查询、OSS 临时凭证和录题异步任务等能力，并参与录题 OpenAPI 契约完善、错误处理统一及代码审查整改。
