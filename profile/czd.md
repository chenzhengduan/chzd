...
- 熟悉基于 Fetch + ReadableStream 的 SSE 流式交互，能够处理生成进度、超时、中断、重试、断线恢复和结果回填。
...
- 负责核心前端及部分 Java 后端研发，交付配置式/对话式 AI 出卷、试卷管理、在线答题与批改、单题及整卷变式、图片/PDF/Word 智能录题等完整业务链路。
- 基于 Vue 3、TypeScript、Pinia 和 Element Plus 搭建核心业务界面，处理学科年级联动、题型分配、试卷生命周期、答题卡联动、答案解析和多主题适配等复杂状态。
- 封装基于 Fetch + ReadableStream 的 SSE 流式交互，支持 POST 流式请求、实时生成进度、超时检测、主动中断、重复请求拦截、错误提示和结果回填。
- 完成数学公式、题组、多空填空、子题、答案解析和题内图等复杂试题结构的统一渲染，保障生成、预览、在线答题、批改和打印场景的一致性。
- 打通图片、PDF、Word 录题流程，覆盖 OSS 直传、异步识别任务、轮询恢复、跨页题内图裁剪、识别结果复核、答案重生成和批量入库。
- 参与 Spring Boot WebFlux 后端接口和数据结构建设，实现试卷/答卷查询、OSS 临时凭证、录题异步任务等能力，并推动 OpenAPI 契约、错误处理和代码审查规范落地。
